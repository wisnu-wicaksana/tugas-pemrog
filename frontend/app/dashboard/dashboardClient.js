...
export default function DashboardClient({ topAnime, topManga, topCharacters }) {
  ...
  const heroItems = [
    ...
    topCharacters?.data?.[0]
      ? {
          type: "character",
          data: { ...topCharacters.data[0], title: topCharacters.data[0].name },
        }
      : null,
        ...
        <div className="space-y-12">
          {!searchTerm && heroItems.length > 0 && (
            <section className="relative h-80 md:h-[450px] rounded-2xl overflow-hidden">
              {heroItems.map((item, index) => (
               ...
               ${
                ...
                >
                  <Image
                    src={
                      item.data.images?.webp?.large_image_url ||
                  ...
                  <div className="relative h-full flex flex-col justify-end p-10 md:p-12">
                    ...
                    <h1 className="text-3xl md:text-5xl font-extrabold text-white leading-tight shadow-black/50 [text-shadow:0_2px_4px_var(--tw-shadow-color)]">
                      {item.data.title}
                    </h1>
                    <p className="text-gray-300 mt-2 max-w-2xl line-clamp-2 md:line-clamp-3 text-sm md:text-base">
                      {item.data.synopsis ||
                        `Karakter favorit dengan ${item.data.favorites?.toLocaleString()} suara.`}
                    </p>
                    <Link
                      href={`/detail/${item.type}/${item.data.mal_id}`}
                      className="mt-6 inline-block bg-blue-500 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-600 transition-transform hover:scale-105 shadow-lg"
                    >
                      Lihat Detail
                    </Link>
                  </div>
              ...
              {/* Tombol Navigasi Bar yang Modern & Responsif */}
              <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center space-x-2">
                {heroItems.map((_, index) => (
                  ...
                  ${
                    index === currentHeroIndex
                      ? "w-8 sm:w-10 bg-white" // Lebar saat aktif
                      : "w-4 sm:w-5 bg-white/40 hover:bg-white/70" // Lebar saat tidak aktif
                  }`}
                    title={`Lihat slide ${index + 1}`}
                  ></button>
                ))}
              </div>
            </section>
              ...
              {isSearching ? (
                ...
                <div className="space-y-12">
                  <section>
                    <SectionHeader
                      title="Anime"
                      href={`/search/anime/${searchTerm}`}
                    />
                    <AnimeList
                      anime={searchResults.anime}
                      favorites={favorites}
                  ...
                  <section>
                    <SectionHeader
                      title="Karakter"
                      href={`/search/character/${searchTerm}`}
                    />
                    <CharacterList
