// file: frontend/app/allTopManga/page.js
'use client';
...
export default function AllTopMangaPage() {
  ...
  const fetchInitialData = () => {
    setLoading(true);
    getApi('top/manga', `page=1&limit=20`)
      .then((res) => {
        setManga(res.data || []);
  ...
  const handleLoadMore = async () => {
    ...
    setLoading(true);
    const nextPageData = await getApi('top/manga', `page=${page}&limit=20`);
    if (nextPageData?.data) {
      setManga(prev => [...prev, ...nextPageData.data]);
      setPage(prev => prev + 1);
  ...
  return (
    <div className="bg-gray-950 min-h-screen">
      ...
      <main className="p-4 sm:p-6 lg:p-8">
        <h1 className="text-3xl font-bold text-white mb-6">Top Manga Terpopuler</h1>
